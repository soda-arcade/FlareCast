...
int CALLBACK WinMain(_In_ HINSTANCE hInstance, _In_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd)
{
    ...
    wc.lpszClassName = _T("FlareCast");
    wc.hIconSm = NULL;
    ::RegisterClassEx(&wc);
    HWND hwnd = ::CreateWindow(
        wc.lpszClassName, _T("FlareCast"), WS_OVERLAPPEDWINDOW,
    ...
    HostSettingsWidget hostSettingsWindow(g_hosting, [&hwnd](bool isRunning) {
        SetWindowTextW(hwnd, isRunning ? L"⚫ [LIVE] FlareCast" : L"FlareCast");
        });
    ...
    bool showInfo = false;
    bool showLogin = Cache::cache.showLogin;
    ...
    t = thread([&]() {
        ...
        Cache::cache.showLogin = !g_hosting.getSession().isValid();
        t.detach();
        });
    ...
    while (!done)
    {
        ...
        if (Cache::cache.showLogin) {
            loginWindow.render(Cache::cache.showLogin);
        }
        else
        {
