...
ACommand* ChatBot::identifyUserDataMessage(const char* msg, Guest& sender, bool isHost) {
	...
	*/
	...
	if (isCommand(msg, CommandFF::prefixes())) {
		return new CommandFF(msg, sender, _gamepadClient, _hotseat);
	}
	...
	if (isCommand(msg, CommandMirror::prefixes())) {
		return new CommandMirror(msg, sender, _gamepadClient);
	}

	if (isCommand(msg, CommandPads::prefixes())) {
		return new CommandPads(msg, sender, _gamepadClient);
	}

	if (isCommand(msg, CommandSpectate::prefixes())) {
		return new CommandSpectate(msg, sender, _guests, _gamepadClient, _hotseat);
	}

	if (isCommand(msg, CommandSwap::prefixes())) {
		return new CommandSwap(msg, sender, _gamepadClient);
	}

	if (isCommand(msg, CommandTriangle::prefixes())) {
		return new CommandTriangle(msg, sender, _gamepadClient, _macro);
	}
