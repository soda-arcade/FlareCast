...
bool GuestListWidget::render(bool& showWindow) {
    ...
    for (int i = 0; i < IM_ARRAYSIZE(tabs); i++) {
        ImGui::PushStyleColor(ImGuiCol_Text, (i == activeTab) ? AppColors::tabActiveText : AppColors::tabText);

        if (ImGui::BeginTabItem(tabs[i].name)) {
            activeTab = i;  // Set selected tab
            tabs[i].renderFunc();  // Call corresponding render function
            ImGui::EndTabItem();
        }
...
void GuestListWidget::renderOnlineGuests() {
    ...
    for (size_t i = 0; i < _guests.size(); ++i) {
        ...
        if (m.congested == 2) {
            ...
            ImGui::PushStyleColor(ImGuiCol_Text, AppColors::positive);
        }
        ...
        ImGui::SameLine();
        ImGui::Image(AppIcons::userOn, ImVec2(16, 16), ImVec2(0, 0), ImVec2(1, 1), AppColors::panelText);

        if (Cache::cache.modList.isModded(userID) || _hosting.getHost().userID == userID) {
            ImGui::SameLine();
            ImGui::Image(AppIcons::hammerIcon, ImVec2(16, 16), ImVec2(0, 0), ImVec2(1, 1), AppColors::panelText);
        }
        ImGui::SameLine();
        if (Cache::cache.vipList.isVIP(userID)) {
            ImGui::SameLine();
            ImGui::Image(AppIcons::star, ImVec2(16, 16), ImVec2(0, 0), ImVec2(1, 1), AppColors::panelText);
        }
        ImGui::SameLine();
        if (MetadataCache::isSpectating(userID)) {
            ImGui::SameLine();
            ImGui::Image(AppIcons::eye, ImVec2(16, 16), ImVec2(0, 0), ImVec2(1, 1), AppColors::panelText);
        }
        ImGui::SameLine();
        if (Config::cfg.hotseat.enabled && Hotseat::instance.inSeat(userID)) {
            ImGui::SameLine();
            ImGui::Image(AppIcons::hotseat, ImVec2(16, 16), ImVec2(0, 0), ImVec2(1, 1), AppColors::panelText);
        }
        ImGui::Unindent(10);
